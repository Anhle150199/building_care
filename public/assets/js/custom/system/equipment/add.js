                            ...
                            console.log("validated!"),
                                "Valid" == t ? (i.setAttribute("data-kt-indicator","on"),
                                    ...
                                    (type = $("#add_department_form_type").val(),
                                        type == "new" ?
                                        $.ajax({
                                            url:$("#kt_toolbar_container").data('route-new'),
                                            type: "post",
                                            data: {
                                                '_token': token,
                                            ...
                                            success: function (data) {
                                                console.log(data);
                                                const newRow = table.row.add( [colIndex(data.id), data.name, data.time, colEnd(data.id, data.name)] ).node()
                                                newRow.id = 'row_'+data.id;
                                                $(newRow).attr('data-id', `${data.id}`);
                                                $(newRow).find('td')[3].className = 'text-center';
                                                table.draw();
                                                reloadDropdown();
                                        ...
                                        $.ajax({
                                            url: $("#kt_toolbar_container").data('route-edit'),
                                            type: "put",
                                            data: {
                                                '_token': token,
                                                name: $("#department_name").val(),
                                            ...
                                            dataType: "json",
                                            success: function (data) {
                                                const editRow = table.row('#row_'+data.id).data([colIndex(data.id), data.name, data.time, colEnd(data.id, data.name)]).node();
                                                editRow.id = 'row_'+data.id;
                                                $(editRow).attr('data-id', `${data.id}`);
                                                $(editRow).find('td')[3].className = 'text-center';
                                                table.draw();
                                                reloadDropdown();
                                                i.removeAttribute("data-kt-indicator"),
                                                    (i.disabled = !1),
